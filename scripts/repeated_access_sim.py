...
#
# 왜 하필 21회인가?
#   서버 설정(config.py)의 PAGE_ACCESS_ALERT_THRESHOLD 기본값이 20이다.
#   즉 "같은 IP가 같은 페이지를 60초 안에 20번 넘게 요청하면 의심한다"는
#   뜻이므로, 그 기준을 "넘기는" 지점까지 확인하려면 20+1=21번을 보내야
#   한다. 로그인 실패 잠금 검증과 같은 이유로 "초과"를 기준으로 삼는다:
#   정상 사용자가 실수로 몇 번 더 새로고침해도 곧바로 잠기지 않도록 여유를
#   주기 때문이다. --attempts로 이 값을 바꾸면 검증하려는 횟수도 함께
#   바뀌므로, 서버의 PAGE_ACCESS_ALERT_THRESHOLD 값과 맞춰서 사용해야 한다.
#
# 안전 원칙:
...
def normalize_path(path: str) -> str:
    ...
    사용자가 --path login 처럼 앞의 슬래시(/)를 빼먹고 입력해도
    자동으로 "/login"으로 바꿔줘서, 잘못된 주소로 요청이 나가는
    실수를 방지한다.
    """
...
def run(
    ...
    # 요청마다 받은 HTTP 상태 코드(200, 302 등)를 종류별로 몇 번씩
    # 받았는지 세어둔다. 나중에 "200이 몇 번, 302가 몇 번" 식으로 한눈에
    # 요약해서 보여주기 위한 용도다.
