...
- **관리자 대시보드** — 세션 로그인으로 보호되는 별도 화면에서 최근 로그인 시도, 현재 잠긴 IP, 관리자 로그인 기록을 실시간(폴링) 확인 + "즉시 해제" 버튼으로 수동 잠금 해제
...
| `/login` | 감시 대상 로그인 — 이 화면에서의 실패 시도가 탐지 대상 |
| `/admin/login` | 관리자 로그인 |
| `/dashboard` | 관리자 대시보드 (로그인 필요) |
